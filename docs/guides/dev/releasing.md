...
Pick the new version by what changed for people importing `dlk`:

- `make version-patch` for bug fixes and internal changes, 
- `make version-minor` for new public API, and 
- `make version-major` for a breaking change to existing API.

To see the transition before anything is written, run `uv version --bump patch --dry-run` first; it reports the same line while leaving `pyproject.toml`, `uv.lock`, and the virtual environment untouched.
...
```sh
claude --model=haiku --effort=high "Please use the skill write-release-notes, performed at step 4 of the guide docs/guides/dev/releasing.md."
```
...
??? info "About the agent's workflow"
	...
	which detects the commit range, drops the version-bump commits, and fills in the frontmatter, one bullet per commit, and the `## Changed files` histograms. Beyond that file and the two generated index pages below, the skill grants itself no permission, so the commit in Step 5 stays yours.

	Once the notes are final, it collects them into the index pages with

	```sh
	.agents/skills/write-release-notes/scripts/collect_changelog.sh
	```

	Run that yourself whenever the notes changed outside the skill, for instance after rewriting a `## Summary` by hand or after backfilling notes for an older tag. It rewrites `CHANGELOG.md` and `docs/releases/index.md` from every file in `docs/releases/`, so the two never drift apart. `--check` reports whether either is stale and writes nothing, and `--print` sends both to stdout.

	Because the `## Summary` is lifted verbatim into pages that carry no changelog, a sentence there pointing at "the changelog below" reads as a dangling reference. Keep the Summary self-contained.

### Step 5: Review and commit six files
...
Six files must change, and a seventh would be a red flag:

1. `pyproject.toml`, where `project.version` moves.
2. `uv.lock`, where the root package entry moves. `uv version` re-locks and re-syncs the project, which is why this stays consistent without a separate `uv lock` run. The Makefile passes `--no-sync`, so the bump re-locks without rebuilding the virtual environment, and whichever accelerator group you synced survives the release.
3. `CITATION.cff`, where `version` and `date-released` move. The `cff-version` line must be untouched; the synchronizing `sed` is anchored to avoid it.
4. `CHANGELOG.md`, gaining one entry for the new version,
5. `docs/releases/index.md`, mirroring the `CHANGELOG.md`, and
6. `docs/releases/v<version>.md`, the new release notes from Step 4.

Both `CHANGELOG.md` and `docs/releases/index.md` are generated by a script (`collect_changelog.sh`); a manual edit here will be lost on the next release.
...
git add pyproject.toml uv.lock CITATION.cff CHANGELOG.md \
    docs/releases/index.md "docs/releases/v$new.md"
