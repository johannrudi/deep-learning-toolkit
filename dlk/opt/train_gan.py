...
def train_epochs(
    ...
    with tqdm(range(n_epochs), desc="epochs", disable=tqdm_disable()) as pbar:
        for epoch_idx in pbar:
            ...
            # call validation function (main process only, with unwrapped models)
            ...
            # stop the whole per-epoch body's timer, then update the epoch-level log
...
def train_batches(
    ...
    for batch_idx, data in enumerate(dataloader):
        ...
        with record_function(RecordFunctionName.LOG):
            ...
            monitor.batch_update(batch_dlog, batch_idx, dlog_item)
